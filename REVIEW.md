...
| C12 *(added 2026-08-14, typed while queueing the ENV02 rebuild; **NOT FIXED — awaiting review**)* | `experiments/v2/ENV02_word_world_map/run.py:236-240` | **The policy anchor the file documents is never applied.** The docstring says W-B/W-B′ are "policy anchored to the base pick distribution (same anchor machinery, item columns)"; `CONFIG` carries `anchor_coef: 1.0`; `item_cols` is computed one line above the call — and then `train_sft(...)` is invoked with neither `move_anchor=` nor `anchor_coef=`, leaving `item_cols` dead. `train_sft` defaults `move_anchor=None`, so the narration SFT trained with **no restoring force on the policy at all**. Every grid-world sibling passes it (`E13:302`, `E14:356`, `E16:534`, `E17:263`); ENV02 is the only build that does not. This is a sufficient explanation for the failure that killed the run: W-B/W-B′ `share_vs_base` came back 0.42–1.52 against the ±0.15 invariance band, which is precisely what an unanchored narration SFT produces. *(Second, independent defect in the same run, listed here because it changes what the rebuild must fix: `narration` and `contingency` are **0.0 for every kind and seed**, W-B included — the generations are bare picks (`'dolm'`, `'brint.'`), so no narration axis was installed either. A uniformly "gentler" recipe — HANDOFF Addendum 5 item 6's phrasing — would fix the drift and make this worse. The two axes want opposite moves, which is README open problem 2 reappearing in the second world.)* | **documented-but-unwired parameter; dead local as the tell** |

| C13 *(added 2026-08-14, from running the gate on a fresh sandbox; **NOT FIXED — awaiting review**)* | `scripts/sync_to_sandbox.sh` (payload) + `tests/test_review_fixes.py` | **Two tests are unrunnable on a freshly synced box, and the suite does not say so.** The sync payload excludes `results/` (correctly — it is output, and shipping it is what once broke the script with "Argument list too long"), but `test_score_e16_scores_the_committed_json` and `test_rescore_manipulation_reports_e17_per_arm` take committed results JSON as their fixture. On a box carrying an older `results/` the suite reports **205 passed**; on a fresh one, **203 passed + 2 FileNotFoundError**. The headline gate number is therefore a property of the machine rather than of the tree, and a fresh sandbox silently loses the two tests that pin C4 (scorer smoke) and C9 (the "score it yourself" reader path). Same genre as the defect this script's own header documents at length — `tests/` and `scripts/` were added to the payload because the gate was unrunnable — one layer further down. Fixing it means either shipping those two fixture JSONs or skipping when the fixture is absent; skipping silently is the worse option, since these two are the regression tests for C4 and C9. | **fixture not shipped; gate number is machine-dependent** |
